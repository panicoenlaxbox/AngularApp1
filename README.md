...
**Development**: the Angular dev server runs on its own port. `proxy.conf.json` proxies `/api/*` requests to the ASP.NET server, so Angular components call `/api/...` without hardcoding any URL. CORS is enabled on the API to allow cross-origin requests from the Angular dev server.
